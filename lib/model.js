...
const { get, set, isEmpty, isArray, merge, each } = require('lodash')
 ...
 * @constructor
 */
function DefaultSchema(options) {
  ...
  each(extendedAttributes, extendedAttribute => {
    set(schemaDefinition, extendedAttribute.name, extendedAttribute.schema)
  })
...
function initializeDefaultSchema(options) {
  const defaultSchema = addNewPathsInSchema(new DefaultSchema(options), options)
  const schema = new Schema(defaultSchema, { id: true, versionKey: false })

  each(options.hooks, hook => {
    schema[hook.hookType](hook.triggedMethod, hook.method)
  })
