...
const { set, get, map, omit, startCase, camelCase, compact, concat, isEmpty, isNumber, each } = require('lodash')
...
class DiffHelper {
  ...
  static getDiff(diffParams = {}, schema = {}, options = {}) {
    ...
    console.log(`getDiff for ${old._id}`);
  ...
  static _mapItems(diffs, schema, options) {
    ...
    function _addItem(item) {
      ...
      if (get(options, 'enabledOps', ['EDITED']).includes(ops.toUpperCase())) {
  ...
  /**
 * @author Allan Naranjo
 * @description Use util to turn object to string at X levels
 * @param { * } value - Value for comparison
 * @return { string } - return is string
 */
  static _ObjectToString(obj) {
    try {
      const stringifyedObject = JSON.stringify(obj);
      return stringifyedObject
    } catch (error) {
      return util.inspect(obj, showHidden = false, depth = 2, colorize = false)
    }
  }
  ...
  static _changeTransformToView(change = {}, options = {}) {
    ...
    set(newChange, `${get(options, 'changeTransform.to') || 'to'}`, DiffHelper._ObjectToString(get(change, 'to')))
    set(newChange, `${get(options, 'changeTransform.from') || 'from'}`, DiffHelper._ObjectToString(get(change, 'from')))
