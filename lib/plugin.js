...
const { isEmpty, get, assign, pick, set, each } = require('lodash')
 ...
 * @param { Object } options - params configuration before saved log
 */
async function _saveHistoryLog(object, options, current) {

  if (isEmpty(get(object, 'changes'))) {
    console.warn(`MONGOOSE-HISTORY-TRACE: path: {changes} no diffs in documents ${object.documentNumber}.`)
    return
  }

  const ChangeLogHelper = options.changeLogHelper;
  const extendedAttributes = get(options, 'extendFromParent', []);

  each(extendedAttributes, extendedAttribute => {
    const targetAttribute = get(current, extendedAttribute.path)
    set(object, extendedAttribute.path, targetAttribute);
  })

  
  ChangeLogHelper?.getActionName(object.action);
  ChangeLogHelper?.setPlacementChangesAdditionalData({ historyData: object })
  ChangeLogHelper?.broadcastChange({ entityChange: object })
  ...
  // each(extendedAttributes, extendedAttribute => {
  //   const targetAttribute = get(current, extendedAttribute.targetInParent || extendedAttribute.path)
  //   set(object, extendedAttribute.path, targetAttribute);
  // })
...
async function _processCreateHistory(params, options) {
  ...
  await _saveHistoryLog(docMapped, options, current)
}
...
async function _processPreUpdate(options) {
  ...
  const mountedDocToSave = await mountDocToSave.call(this, { options }, 'updated');

  if (!mountedDocToSave) {
    console.warn(`Trying to fire save hook to an unexisting document with query ${this._conditions}.`)
    return Promise.resolve()
  }
